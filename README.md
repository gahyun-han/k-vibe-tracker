...
- `/[locale]/profile`: Supabase auth-backed profile when credentials exist, plus a guest-mode dashboard with the active local persona, an Instagram-style local saved-place grid with a root-style See all toggle that opens the in-app place detail sheet, current route progress/next stop, a tappable My Routes card and actions, and localized app settings state when Supabase is not configured.
