...
- `/[locale]/map`: nearby K-vibe places. It requests browser geolocation, restores the last known GPS position for up to 30 minutes, falls back to Seoul with localized warning toast feedback when location is unavailable, calls `/api/places`, caches same-query place responses locally for 1 hour, shows a localized nearby-spots result header for the current filters, includes empty-result recovery actions for resetting filters or refreshing location plus localized related-search chips, lazy-loads `/api/places/[contentId]` details and image galleries for selected pins, renders Kakao Maps when `NEXT_PUBLIC_KAKAO_MAP_KEY` exists, otherwise uses the no-cost local map preview, exposes the root S3 SNS analyzer FAB, and can add or share a selected place through local route/detail URLs with shared toast feedback. Place detail sheets also show S4-style local Seen in badges for YouTube and Instagram signals without calling live SNS providers.
