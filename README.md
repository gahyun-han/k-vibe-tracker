...
- `/[locale]/persona`: K-content route generator. It guides users through a localized 3-step theme, mood, and confirmation flow, calls `/api/routes/generate` with the active locale, renders a localized local route preview, and can save the plan into `localStorage`.
