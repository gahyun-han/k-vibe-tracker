...
- `/[locale]/analyze`: SNS URL analyzer. It detects YouTube and Instagram links, presents platform-aware example cards, immediately runs local/mock analysis for YouTube examples, calls `/api/analyze` for YouTube with the active locale, shows a localized 4-step loading state, caches same-video analysis results locally for 1 hour with cache-hit and failure toast feedback, returns localized local mock spot extraction by default, shows accessible confidence bars on detected spot cards, offers a sample YouTube retry action for empty results, lets detected spot cards open the map with the place detail sheet already focused, can draft a local route from detected places with shared toast feedback, and keeps live Instagram extraction deferred until an approved no-cost/provider path exists.
