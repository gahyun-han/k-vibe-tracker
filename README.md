...
- `/[locale]`: actionable home feed with a root S1-style visible language grid, root-aligned story topic filters with accessible selected-state semantics, TourAPI-backed Seoul feed cards, localized crowd badges, 1-hour local feed caching with retry/fallback states, localized offline/install prompts, localized empty-state recovery actions, local save controls with shared toast feedback, feature shortcuts, and trend chips that open focused map views. Feed card image/text taps now open the in-app map detail sheet with TourAPI detail and crowd context when available, and saved persona preferences locally select a matching feed category.
- `/[locale]/map`: nearby K-vibe places. It requests browser geolocation, restores the last known GPS position for up to 30 minutes, falls back to Seoul with localized warning toast feedback when location is unavailable, calls `/api/places`, caches same-query place responses locally for 1 hour with localized cache/error toast feedback on refresh failures, shows a localized nearby-spots result header for the current filters, includes empty-result recovery actions for resetting filters or refreshing location plus localized related-search chips, exposes selected-state semantics on category filter chips, lazy-loads `/api/places/[contentId]` details and image galleries for selected pins, renders Kakao Maps when `NEXT_PUBLIC_KAKAO_MAP_KEY` exists, otherwise uses the no-cost local map preview, exposes accessible pin labels with place, category, and distance context, exposes the root S3 SNS analyzer FAB, and can add or share a selected place through local route/detail URLs with shared toast feedback. Place detail sheets also show S4-style local Seen in badges for YouTube and Instagram signals without calling live SNS providers.
- `/[locale]/analyze`: SNS URL analyzer. It detects YouTube and Instagram links, presents platform-aware example cards, immediately runs local/mock analysis for YouTube examples, calls `/api/analyze` for YouTube with the active locale, shows a localized 4-step loading state, caches same-video analysis results locally for 1 hour with cache-hit and failure toast feedback, returns readable localized local mock spot extraction by default, shows accessible confidence bars and estimated-location labels on detected spot cards, covers result/source/action labels with exact multilingual copy tests, offers a sample YouTube retry action for empty results, lets detected spot cards open the map with the place detail sheet already focused, can draft a local route from detected places with shared toast feedback, and keeps live Instagram extraction deferred until an approved no-cost/provider path exists.
- `/[locale]/persona`: K-content route generator. It guides users through a localized 3-step theme, mood, and confirmation flow with six root-aligned persona themes, can save that selection as a no-cost local feed preference with localized toast feedback, covers key generator/action/theme labels with exact multilingual copy tests, calls `/api/routes/generate` with the active locale, renders a localized local route preview, and can save or share the plan with shared toast feedback before editing it from `localStorage`.
- `/[locale]/route`: editable route timeline. It reads and writes the saved route plan in `localStorage`, restores no-cost `route=` share URLs with localized warning recovery when a shared payload is invalid, shows the generated route summary in the S9 header, tracks completed stops locally, supports drag reorder, localized icon move controls, removal, localized sample stop insertion, localized route action/timeline/current-distance copy, route action toast feedback, a local route mini map, S9-style whole-route Open in Map handoff, user-clicked next-stop distance checks, free local walking/transit travel hints between stops, in-app stop detail handoff to the map sheet, Google Maps walking handoff links, and same-origin URL sharing without a public-link backend.
- `/[locale]/docent`: no-cost local docent. It opens a selected route stop with localized structured captions, local voice controls, fallback/script text, a localized script progress bar, browser `speechSynthesis` voice playback with active script-section highlighting when supported, and a user-clicked 100m arrival check that can auto-start local browser speech when coordinates are in range, instead of a paid TTS API.
- `/[locale]/radar`: convenience facility radar. It requests browser geolocation, restores the last known GPS position for up to 30 minutes, falls back to Seoul with localized warning toast feedback when location is unavailable, calls `/api/facilities`, caches same-query facility responses locally for 1 hour with localized cache/error toast feedback on refresh failures, supports radius/type filtering with shared lucide facility icons including the root S11 ATM, medical, and transit needs, offers an empty-state radius expansion action, enriches popup facilities from TourAPI `searchFestival2` when `TOUR_API_KEY` is configured, shows a no-cost radar map preview, and can open selected facilities in Google Maps from either the card header quick action or expanded card action after a user click.
- `/[locale]/profile`: Supabase auth-backed profile when credentials exist, plus a guest-mode dashboard with the active local persona, an S12-style sign-in prompt when account sync is available, an Instagram-style local saved-place grid with a root-style See all toggle that opens the in-app place detail sheet, a no-saved-places map CTA, current route progress/next stop, a tappable My Routes card and actions, exact multilingual copy tests for the guest dashboard CTAs and route labels, and localized app settings state when Supabase is not configured.
...
Map categories, live Kakao pin labels, fallback map pins, search/result labels, recovery actions, Radar facility filters, and Place Detail save/share/docent/social-proof/category badges use the active locale's copy instead of hardcoded English category text.

The shared app shell includes a Mobile/PC view switcher. Mobile mode keeps the bottom tab bar from the root wireframes and emphasizes Analyze as the central SNS spot CTA; PC mode uses a left rail and wider content area so map, route, and profile workflows get more horizontal space. The home entry exposes the same switcher, defaults desktop-width sessions to PC mode, and stores the user's choice locally.

The app includes a production-only PWA runtime: `components/common/PwaRuntime.tsx` updates the document language from the active locale, and `public/sw.js` provides a static app-shell cache for manifest/icons/static chunks plus the main localized app routes with locale-aware same-origin navigation fallback. `app/[locale]/error.tsx` provides a localized segment-level error recovery screen, while `components/common/NetworkStatusBanner.tsx` shows a localized offline-mode banner when the browser reports a network disconnect. `components/common/PwaInstallPrompt.tsx` shows a localized home-screen install prompt only when the browser reports install eligibility.
...
- Sprint 1 is in progress:
  ...
  - Home, Map, and Radar now share a 1-hour local API response cache so same-query place/facility results can render immediately and survive temporary fetch failures.
  ...
  - Map place detail save, unsave, add-to-route, share, and copy outcomes now raise the shared accessible toast feedback in the active locale.
  ...
  - Analyze route drafting plus Persona route generation/save/share/personalize actions now raise shared accessible toast feedback in the active locale.
  ...
  - Route header now exposes the generated route summary, and icon move controls use locale-aware accessible labels and tooltips.
  - Route page now includes a user-clicked current-distance check to the next incomplete stop using browser geolocation and local Haversine math, without automatic GPS polling.
  ...
  - Route page action outcomes now use the shared accessible toast system for reorder, remove, completion, sharing, directions, and guidance-blocked feedback, with localized dismiss labels.
  - `/api/analyze` is now local-first and gated behind `ENABLE_AI_WORKER_ANALYSIS` for worker calls.
  - Analyze page now has English local-first copy, mock/source indicators, cleaner result cards, and accessible confidence bars.
  - Home, Analyze, and Radar empty states now expose no-cost recovery actions that match the UI design guidance: feed reset/map exploration, sample YouTube retry, and radius expansion.
  ...
  - Home/Landing entry copy is repaired for Korean, Japanese, and Simplified Chinese, with exact assertions for language/start/trending labels, story filters, nav labels, and key category labels.
  ...
  - The in-app feature guide now includes per-feature shortcut actions instead of only static descriptions, restores focus after closing, exposes dialog/open state to assistive tech, renders the feature shortcuts as an ordered list, and its sheet scrolls safely on small mobile screens with longer localized copy.
  ...
  - Docent now includes a user-clicked 100m arrival check that uses route/map coordinates, browser geolocation, and local Haversine distance calculation, then auto-starts local browser speech when in range without background GPS polling or paid services.
  - Profile now works as a guest-mode dashboard with local saved places, a compact saved-place preview with a See all toggle, a current route progress card, next-stop context, and My Routes actions without needing Supabase.
