...
- `/[locale]`: actionable home feed with language selection, root-aligned story topic filters, TourAPI-backed Seoul feed cards, localized crowd badges, 1-hour local feed caching with retry/fallback states, local save controls, feature shortcuts, and trend chips that open focused map views. Feed card image/text taps now open the in-app map detail sheet with TourAPI detail and crowd context when available, and saved persona preferences locally select a matching feed category.
