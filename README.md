...
- `/[locale]/analyze`: YouTube URL analyzer. It calls `/api/analyze` with the active locale, returns localized local mock spot extraction by default, can open detected spots on the map, can draft a local route from detected places, and only calls an AI worker when explicitly enabled.
