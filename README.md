...
- `/[locale]/map`: nearby K-vibe places. It requests browser geolocation, restores the last known GPS position for up to 30 minutes, falls back to Seoul with localized warning toast feedback when location is unavailable, calls `/api/places`, caches same-query place responses locally for 1 hour with localized cache/error toast feedback on refresh failures, shows a localized nearby-spots result header for the current filters, includes empty-result recovery actions for resetting filters or refreshing location plus localized related-search chips, lazy-loads `/api/places/[contentId]` details and image galleries for selected pins, renders Kakao Maps when `NEXT_PUBLIC_KAKAO_MAP_KEY` exists, otherwise uses the no-cost local map preview, exposes the root S3 SNS analyzer FAB, and can add or share a selected place through local route/detail URLs with shared toast feedback. Place detail sheets also show S4-style local Seen in badges for YouTube and Instagram signals without calling live SNS providers.
- `/[locale]/analyze`: SNS URL analyzer. It detects YouTube and Instagram links, presents platform-aware example cards, immediately runs local/mock analysis for YouTube examples, calls `/api/analyze` for YouTube with the active locale, shows a localized 4-step loading state, caches same-video analysis results locally for 1 hour, returns localized local mock spot extraction by default, shows accessible confidence bars on detected spot cards, offers a sample YouTube retry action for empty results, lets detected spot cards open the map with the place detail sheet already focused, can draft a local route from detected places with shared toast feedback, and keeps live Instagram extraction deferred until an approved no-cost/provider path exists.
