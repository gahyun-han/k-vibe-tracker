...
- Responsive app navigation lives in `components/layout/BottomNav.tsx`: it renders the bottom tab bar in Mobile mode with Analyze emphasized as the central SNS spot CTA, and a left navigation rail in PC mode.
- Top navigation lives in `components/layout/TopBar.tsx` and exposes the language switcher, Mobile/PC view switcher, and account entry. View mode is stored in `localStorage` under `k-vibe-view-mode`; new desktop-width sessions default to PC mode, while phone-width sessions default to Mobile mode.
...
- The feature guide button lives in `components/common/TutorialButton.tsx` and is mounted by the home page plus `components/layout/AppLayout.tsx` on the main app screens. Each guide step includes a localized shortcut into the related workflow, the trigger exposes dialog/open state, the steps render as an ordered feature list, shortcut actions include feature-specific accessible labels, focus moves to the close action while open and returns to the trigger on close, Tab and Shift+Tab stay trapped inside the dialog, and the sheet uses a viewport-bounded scroll area so longer localized copy remains reachable on mobile.
...
- PWA install UI lives in `components/common/PwaInstallPrompt.tsx` and is mounted by the home page plus `components/layout/AppLayout.tsx`. It appears only when the browser emits `beforeinstallprompt`, calls the browser install prompt after a user tap, and stores dismissals in `localStorage`.
...
- The home entry at `/[locale]` presents a root S1-style visible language grid, local-first status, Mobile/PC view switcher, root S2-style story topic filters, TourAPI-backed Seoul feed cards with localized crowd badges, feature shortcuts, and trend chips that open focused map views. In PC mode the feed and feature shortcuts use wider grids; Mobile mode keeps the compact phone-scroller layout. Feed card image/text taps follow the root S2 flow by opening `/[locale]/map` with `detail=1` and showing the local place detail sheet. Empty filtered feeds show localized recovery actions to reset to all places or continue exploration on the map.
...
- Route stops and the primary guidance action can open `/[locale]/docent` for local voice captions, script progress, and a user-clicked 100m arrival check that can auto-start browser speech when stop coordinates are in range.
...
- Profile stays usable without Supabase credentials, shows an S12-style localized sign-in prompt when account sync is configured but the user is still in guest mode, shows local saved places in a root S12-style square visual grid with a localized See all/Show less toggle that opens the S4-style place detail sheet, shows the current local route with a tappable route card, route progress, next-stop context, and Continue/Edit actions, and exposes localized settings rows for language, notifications, offline maps, and map data source state. Guest dashboard, saved-place, saved-route, persona, and route-progress labels are covered by exact localized UI-copy tests for `ko`, `en`, `ja`, and `zh`.
...
- Home feed requests the same `/api/places` contract with the active locale, shows the response source, stores successful responses in the shared local API cache, supports local category filters with accessible selected-state semantics and a localized empty-state reset/map handoff, can save or remove places with shared toast feedback, and hands selected cards to the map with `source=home`, `detail=1`, category/address/tags, crowd level, and TourAPI content identifiers when available.
...
- Map stores successful `/api/places` responses in the shared local API cache and displays cached place lists while a fresh same-query request is pending or if it fails. Failed refreshes now raise localized toasts: a warning when cached places remain visible, or an error when no cached places are available. The bottom sheet includes a localized nearby-spots header with the current filtered result count and a localized empty-result recovery state that can reset filters, trigger a user-clicked location refresh, or swap to localized related-search chips when a typed query has no matches, matching the root S3 "Spots Near You" pattern without another automatic API call.
...
- Current local verification shows the Kakao SDK loads on `http://localhost:3000`, `/ko/map` reaches `data-map-mode="ready"` in Chrome with a nonzero map container, 26 rendered Kakao tile images, 16 visible app pin buttons, and no Kakao console warnings/errors.
...
- Category filters, map list icons, live Kakao overlay labels, fallback pin labels, search/location/result labels, recovery actions, and place detail sheets use locale-aware copy instead of fixed English-only strings. Category filter chips expose pressed state for active selections, and main map pin buttons expose accessible labels with the place name, localized category, and distance context. Exact UI-copy tests cover the high-visibility Map labels for `ko`, `en`, `ja`, and `zh`.
...
- Place detail sheets lazy-load TourAPI `detailCommon2`, `detailIntro2`, and `detailImage2` through the server detail API for overview, images, phone, operating time, rest day, and parking fields. When multiple images are available, the detail sheet exposes a compact image gallery that switches the hero image without another provider call. Route/save/share/docent/detail/image/social-proof/crowd labels are covered by exact localized UI-copy tests for `ko`, `en`, `ja`, and `zh`. The sheet also renders root S4-style Seen in badges for YouTube and Instagram from deterministic local place metadata, so it does not call live SNS APIs or scraping providers.
...
- The Analyze page sends the active URL locale to `/api/analyze`; local mock titles, place names, and reasons use readable Korean, English, Japanese, and Simplified Chinese copy, and enabled AI worker requests receive the locale value for future provider-side localization.
- Analyze result cards follow the root S7 flow: each candidate shows a localized confidence label plus an accessible progress bar, labels low-confidence or coordinate-missing candidates as an estimated location, and selecting a coordinate-backed spot opens `/[locale]/map` with `detail=1`, focuses the analysis location, and opens the local place detail sheet with the analysis reason as context.
...
- Analyze stores successful same-video, same-locale results in the shared 1-hour local API cache. Cache hits skip the network call, show a localized previous-result source label, and raise a localized cache-hit toast; fetch failures raise the localized error toast while keeping the inline retry state.
...
- Facility types cover the root S11 immediate-need set with local restroom, ATM, medical, transit, cafe restroom, pharmacy, convenience, and popup/event helpers. ATM, medical, and transit results are local/mock only until live no-cost sources are approved.
...
- Radar stores successful `/api/facilities` responses in the shared local API cache and displays cached facility lists while a fresh same-query request is pending or if it fails. Failed refreshes now raise localized toasts: a warning when cached facilities remain visible, or an error when no cached facilities are available.
...
- Radar filter chips, facility type names, preview pins, empty-state actions, map labels, and facility cards use shared lucide facility icons plus locale-aware labels so labels stay localized and iconography stays consistent. Exact UI-copy tests cover the high-visibility Radar labels for `ko`, `en`, `ja`, and `zh`.
...
- Route sharing is no-cost and local-first: `lib/routes.ts` encodes the current plan into a same-origin `route=` URL parameter, and `/[locale]/route` restores that payload into the editor before falling back to `localStorage`. Invalid or stale shared payloads now raise a localized warning toast before the local/starter route fallback is shown.
...
- Route editor shows the generated plan summary in the S9 header plus a no-cost local mini map preview. Stops can be reordered through drag-and-drop or localized icon move controls, and route mutations/actions now also raise shared toast feedback for reorder, remove, completion, share/copy, directions, and guidance-blocked states. The sticky action bar stays inside the responsive app shell on mobile and desktop. The root S9 Open in Map CTA opens `/[locale]/map?source=route-map` centered on the first stop without entering detail mode or calling a routing provider. The route screen can also check current distance to the next incomplete stop after a user tap, using browser geolocation plus local Haversine math and no background polling. The timeline now inserts dashed travel segments between stops: short legs stay as walking, while longer legs show a local transit hint using distance thresholds and estimated minutes. Selecting a stop body opens `/[locale]/map` with `detail=1`, focusing that stop, and showing the local place detail sheet with route context. Route action labels, timeline segment labels, current-distance status, and sample-stop copy are localized for `ko`, `en`, `ja`, and `zh` with exact UI-copy tests. Stop pins, explicit Google Maps icons, and the walking directions icon open Google Maps URLs in a new tab only after the user clicks; no Maps Directions API, Kakao Mobility API, or paid route calculation is called.
- Docent playback uses browser `speechSynthesis` with generated captions from the selected route stop. The script is split into intro, details, tags, and next-step sections; the UI shows localized script progress, and speech boundary events highlight and scroll the active section when supported. Local voice controls, captions, fallback stop text, and script templates are localized for `ko`, `en`, `ja`, and `zh` with exact UI-copy tests. It does not call OpenAI TTS or any paid API.
- Docent arrival checking reads `lat` and `lng` from the query string, asks for browser geolocation only after a user tap, computes distance locally with Haversine, treats 100m as the ready radius, and starts local browser speech when supported and idle. Automatic polling, push prompts, and provider TTS remain approval-gated.
...
- Analyze and Radar screen copy is also routed through `lib/ui-copy.ts` so the local-first SNS and facility workflows stay available in all supported locales. Exact UI-copy tests cover high-visibility Analyze labels for SNS platform badges, loading steps, result actions, source badges, confidence/estimated-location text, local-mode messaging, and route-save feedback across `ko`, `en`, `ja`, and `zh`.
- Persona, Route, and Docent screen copy is routed through `lib/ui-copy.ts`, including editor status messages, route stats, CTA labels, local voice controls, docent script templates, persona theme labels, and tutorial shortcut actions.
