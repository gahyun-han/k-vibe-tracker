...
- The feature guide button lives in `components/common/TutorialButton.tsx` and is mounted by the home page plus `components/layout/AppLayout.tsx` on the main app screens. Each guide step includes a localized shortcut into the related workflow, and the sheet uses a viewport-bounded scroll area so longer localized copy remains reachable on mobile.
