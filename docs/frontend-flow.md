...
- Map stores successful `/api/places` responses in the shared local API cache and displays cached place lists while a fresh same-query request is pending or if it fails. The bottom sheet includes a localized nearby-spots header with the current filtered result count, matching the root S3 "Spots Near You" pattern without another API call.
