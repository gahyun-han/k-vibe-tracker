...
- Category filters, map list icons, live Kakao overlay labels, fallback pin labels, and place detail sheets use lucide icons plus locale-aware labels instead of hardcoded English category strings. Category filter chips expose pressed state for active selections, and main map pin buttons expose accessible labels with the place name, localized category, and distance context.
