...
- TourAPI key: provided by the user for local development and stored only in `.env.local`, which is gitignored. The user confirmed all 26 Korea Tourism Organization OpenAPI service applications are approved under the same key. `/api/places`, `/api/places/[contentId]`, and Radar popup enrichment through `searchFestival2` still use mock fallback if the key is absent or TourAPI fails. TourAPI is a public-data API, but local live checks consume the development account request quota; one `/api/facilities` popup verification call, one `/ko/map` page verification, one S4 place-detail gallery verification that opened a local map result plus two place detail sheets, one S3 analyzer-FAB map verification, one S7 analyze-to-map detail handoff verification, one S9 route-stop detail handoff verification, one S2 home-feed detail handoff verification, one S12 profile saved-detail handoff verification, one follow-up `/api/places` Kakao-domain check probe, and one additional Chrome `/ko/map` Kakao-domain verification page load were made on 2026-06-17.
