...
- Kakao Maps JavaScript key: provided by the user for local development and stored only in `.env.local`, which is gitignored. Current verification shows the SDK request is still rejected by Kakao with auth/forbidden responses, so Kakao Developers needs Kakao Map/Local enabled and `http://localhost:3000` registered as an allowed web platform origin before the real SDK can render. The app still has a no-cost fallback and only attempts Kakao Maps when `NEXT_PUBLIC_KAKAO_MAP_KEY` is configured.
