...
| `docs/frontend-structure-validation.md` | boram72/k-vibe 프론트 구조를 현재 Next 앱에 어떻게 반영할지 정리한 기준 문서 |
...
1. `/ko/persona`에서 4개 페르소나를 선택해 하루 루트가 바로 생성되는지 확인합니다.
2. 생성된 루트에서 `루트 편집`을 눌러 `/ko/route`로 이어지는지 확인합니다.
3. 현재처럼 `K-콘텐츠 페르소나 선택`만 남기고 테마 직접 만들기 흐름을 제외해도 되는지 확인합니다.
4. `boram72/k-vibe`는 전체 이전 대상이 아니라 구조 참고용이라는 점을 공유합니다.
5. 다음 우선순위를 정합니다: Supabase 데이터 연결, 루트 편집 강화, SNS 분석 강화, 저장/계정 동기화 중 무엇을 먼저 할지.
